...
export type SupabaseRequestScope = {
  userId?: string | null;
...
function getSupabaseAuthHeaders(supabaseAnonKey: string, scope?: SupabaseRequestScope) {
  return {
    apikey: supabaseAnonKey,
    Authorization: `Bearer ${scope?.accessToken?.trim() || supabaseAnonKey}`,
    ...(scope?.userId ? { "x-notice-user-id": scope.userId } : {}),
  };
}
...
async function ensureUserInSupabase(scope?: SupabaseRequestScope) {
  ...
  const response = await fetch(`${supabaseUrl}/rest/v1/users?on_conflict=id`, {
    ...
    headers: {
      ...
      ...getSupabaseAuthHeaders(supabaseAnonKey, scope),
...
export async function updateLinkStatusInSupabase(
  ...
  applyUserScope(query, scope?.userId);
...
export async function listRecentLinksFromSupabase(limit = 20, scope?: SupabaseRequestScope) {
  if (!hasSupabaseEnv()) {
    return {
      skipped: true,
      reason: "SUPABASE_URL \uB610\uB294 SUPABASE_ANON_KEY\uAC00 \uC124\uC815\uB418\uC9C0 \uC54A\uC558\uC2B5\uB2C8\uB2E4.",
...
export async function listFoldersFromSupabase(scope?: SupabaseRequestScope) {
  if (!hasSupabaseEnv()) {
...
export async function listTasksFromSupabase(limit = 50, scope?: SupabaseRequestScope) {
  if (!hasSupabaseEnv()) {
    return { skipped: true, items: [] };
  }
